...
# Constants / Paths
# -----------------------
HARD_MAX_MISSING = 2  # <-- hardcoded cap: recipes with >2 missing are filtered out
...
def greedy_search(user_ingredients, df, weights, ingredient_col="ingredients",
                  hard_min_matches=2,  # still configurable if you want
...
def parse_args():
    ...
    # NOTE: removed --hard-max-missing (now hardcoded to 3)
...
def main():
    ...
    # Optionally append new tokens to dictionary (keeps it evolving but still dataset-first)
    if args.auto_append_dict:
        dataset_words, added = maybe_append_new_words(user_tokens_all, dict_path, set(dataset_words), dry_run=False)
        if added:
    ...
    print("\n👍👎 Help me learn:")
    print(" - Enter the number of a suggestion you liked to ACCEPT it.")
    print(" - Enter 'r <num>' to REJECT a specific suggestion (e.g., 'r 2').")
    ...
    if fb.lower().startswith("r "):
        try:
            reject = int(fb.split()[1])
        except Exception:
            print("⚠️ Couldn't parse rejection index.")
            return
    else:
        try:
            accept = int(fb)
        except Exception:
            print("⚠️ Couldn't parse acceptance index.")
            return

    if accept is not None:
        if accept < 1 or accept > len(results):
            print("⚠️ Out of range.")
            return
        row = results.iloc[accept - 1]
        ...
        if reject < 1 or reject > len(results):
            print("⚠️ Out of range.")
            return
        row = results.iloc[reject - 1]
        weights = update_weights_for_feedback(
            weights,
            matched_set=row["match_set"],
            missing_set=row["missing_set"],
        ...
        print("✅ Learned from your rejection. Weights updated.")
