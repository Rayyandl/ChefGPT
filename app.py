...
if uploaded_file is not None:
    try:
        ...
        st.success(f"Dataset loaded: {len(df)} recipes")
                ...
                for i, (_, row) in enumerate(results.head(10).iterrows(), start=1):
                    with st.expander(f"{i}. {row.get('name', 'Unknown')}"):
                        ...
                        if col1.button("👍 Good", key=f"good{i}"):
                            weights = update_weights_for_feedback(weights, row["match_set"], row["missing_set"], accepted=True)
                        ...
                        if col2.button("👎 Bad", key=f"bad{i}"):
                            weights = update_weights_for_feedback(weights, row["match_set"], row["missing_set"], accepted=False)
                            save_weights(weights, WEIGHTS_PATH)
